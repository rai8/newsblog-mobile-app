...
const imageProcess = require('../utils/imageProcessing')
...
const createNews = async (req, res) => {
  ...
  try {
    ...
    await news.create(req.body, id, imageName) // http://locahost:3000/image-name
...
const searchPosts = async (req, res) => {
  try {
    ...
    if (query.trim()) {
      ...
      if (response.length === 0) return res.json({ success: false, message: 'No match found..' })
